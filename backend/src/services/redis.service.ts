...
export const getCachedDbCredentials = async( redis: RedisType, databaseId: string ) => {
    const key = `db_cred:${databaseId}`;
    try {
...
export const clearCachedDbCredentials = async( redis: RedisType, databaseId: string ) => {
    const key = `db_cred:${databaseId}`;
    try {
        await redis.del(key);
    } catch (error) {

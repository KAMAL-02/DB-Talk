...
export const cacheDbCredentials = async (
  redis: RedisType,
  databaseId: string,
  schema: any,
) => {
  const key = `db_cred:${databaseId}`;
...
export const getCachedDbCredentials = async (
  redis: RedisType,
  databaseId: string,
) => {
  const key = `db_cred:${databaseId}`;
...
export const clearCachedDbCredentials = async (
  redis: RedisType,
  databaseId: string,
) => {
  const key = `db_cred:${databaseId}`;
...
export const cacheSchema = async (
  ...
  const SCHEMA_TTL = 180 * 60; // 3 hour
  try {
    await redis.set(key, JSON.stringify(schema), "EX", SCHEMA_TTL);
  } catch (error) {
    console.error("Error caching schema:", error);
...
export const clearCachedSchema = async (
  redis: RedisType,
  databaseId: string,
) => {
  const key = `db_schema:${databaseId}`;
